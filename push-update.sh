...
updates_dir=/data/magma_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have Magma-VERSION-DATE-TYPE-* as format"
...
# Assume Magma-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.magma.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.magma.updater/databases/updates.db" \
