...
(based on LineageOS Updater, big thanks to these guys!)


Server requirements
-------------------
The app sends `GET` requests to the URL defined by the `updater_server_url`
resource (or the `magma.updater.uri` system property) and expects as response
...
The `romtype` attribute is the string to be compared with the `ro.magma.releasetype` property.  
The `size` attribute is the size of the update expressed in bytes.  
The `url` attribute is the URL of the file to be downloaded.  
The `version` attribute is the string to be compared with the `ro.magma.build.version` property.  
